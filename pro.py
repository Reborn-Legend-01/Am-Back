...
def login_lagi334():
	try:
		os.system('clear')
		banner()
		cetak(nel('\t                   COOKIES MENU[green] '))
		asu = random.choice([m,k,h,b,u])
		cookie=input(f'{h}[{h}•{h}] ENTER COOKIES :{asu} ')
		...
		print(f'  {u}[{h}•{u}]{h} LOGIN SUCCESSFULL RUN TOOLS AGAIN!!!!{k} ');time.sleep(1)
		exit()
	except Exception as e:
...
#		method.append('touch')
	...
	_jembot_ = input(' SHOW APPLICATIONS LINKED ( y/t ) ')
#	if _jembot_ in ['']:
#		print('➥➥➣Pilih Yang Bener Kontol ')
#		back()
	...
	with tred(max_workers=30) as pool:
		for yuzong in id2:
			...
			if 'mobile' in method:
				...
				pool.submit(crackmbasic,idf,pwv)
	print('')
	cetak(nel('\t[purple]➥➥➣[green] PROCESS DONE [purple] <<[yellow] '))
	print(f'[{h}•{u}]{h} OK : {h}%s '%(ok))
	print(f'{k}[{k}•{h}]{k} CP : {k}%s{u} '%(cp))
	print('')
	print('{k}➥➥➣continue hacking ( Y/t ) ? ')
	woi = input('➥➥➣Pilih : ')
	if woi in ['y','Y']:
		back()
	else:
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r{bo} 🕜{k}{loop}{P}/{h}{len(id)}{H} {P}[{h}{ok}{P}] {P}[{bo}{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
